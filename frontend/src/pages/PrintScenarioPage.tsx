...
export function PrintScenarioPage() {
  ...
  return (
    <div className="max-w-screen-lg mx-auto px-4 py-8 print:px-0">
      ...
      <p className="mt-2 text-sm text-brand-gray-600">{scenario.what_it_proves}</p>
      {scenario.narration && (
        <p className="mt-2 text-sm text-brand-gray-500">{scenario.narration}</p>
      )}
      <div className="mt-6 space-y-4">
        {scenario.variants.map((variant) => (
          <div key={variant.label} className="rounded-lg border border-brand-gray-200 p-4">
